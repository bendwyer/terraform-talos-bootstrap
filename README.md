...
| <a name="input_talos_node_data"></a> [talos\_node\_data](#input\_talos\_node\_data) | Map of Talos nodes by hostname. Contains controlplanes and workers maps where keys are node hostnames and values specify node\_ip (required) and install\_disk (required). | <pre>object({<br/>    controlplanes = map(object({<br/>      node_ip      = string<br/>      install_disk = string<br/>    }))<br/>    workers = map(object({<br/>      node_ip      = string<br/>      install_disk = string<br/>    }))<br/>  })</pre> | <pre>{<br/>  "controlplanes": {<br/>    "controlplane-1": {<br/>      "install_disk": "/dev/sda",<br/>      "node_ip": "10.5.0.2"<br/>    },<br/>    "controlplane-2": {<br/>      "install_disk": "/dev/sda",<br/>      "node_ip": "10.5.0.3"<br/>    },<br/>    "controlplane-3": {<br/>      "install_disk": "/dev/sda",<br/>      "node_ip": "10.5.0.4"<br/>    }<br/>  },<br/>  "workers": {<br/>    "worker-1": {<br/>      "install_disk": "/dev/sda",<br/>      "node_ip": "10.5.0.5"<br/>    }<br/>  }<br/>}</pre> | no |
...
| <a name="input_talos_vip"></a> [talos\_vip](#input\_talos\_vip) | Virtual IP address for control plane high availability. Used as the cluster API endpoint address. | `string` | `null` | no |
